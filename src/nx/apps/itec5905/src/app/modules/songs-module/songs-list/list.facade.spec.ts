...
describe('SongListFacade', () => {
  let facade: SongListFacade;
  let store: Store;
  ...
  describe('used in NgModule', () => {
    beforeEach(() => {
      ...
      facade = TestBed.inject(SongListFacade);
    ...
    test('loadEntities() should return a list of (1) with loading == false and httpClient.get is invoked', async () => {
      ...
      expect(httpClient.get).toBeCalledWith('songs-odata/odata/v1/Songs?&$count=true');
    ...
    /**
     * Use `songsLoaded` to manually submit list for state management
     */
    test('gridData$ should return the loaded list; and loaded flag == true', async () => {
      let list = await readFirst(facade.gridData$);
      expect(list?.data.length).toBe(0);
      store.dispatch(songActionTypes.loadSongsSuccess(createODataResult([createTestSong(), createTestSong()])));
