...
describe('ArtistListFacade', () => {
  ...
  describe('used in NgModule', () => {
    beforeEach(() => {
      ...
      facade = TestBed.inject(ArtistListFacade);
    ...
    test('loadEntities() should return a list of (1) with loading == false and httpClient.get is invoked', async () => {
      ...
      expect(httpClient.get).toBeCalledWith('artists-odata/odata/v1/Artists?&$count=true');
    ...
    /**
     * Use `artistsLoaded` to manually submit list for state management
     */
    test('gridData$ should return the loaded list; and loaded flag == true', async () => {
      let list = await readFirst(facade.gridData$);
      expect(list?.data.length).toBe(0);
      store.dispatch(artistActionTypes.loadArtistsSuccess(createODataResult([createTestArtist(), createTestArtist()])));
